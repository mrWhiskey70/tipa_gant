...
В новой версии добавлен `ErrorBoundary`, `Errors panel` и блок `Статус импорта` с лоадером. Если при импорте есть проблемы с данными, приложение **не падает**: смотрите текущий статус обработки и сообщения в `Errors panel`.
...
- Во время загрузки показывается лоадер и этапы: чтение файла → парсинг/валидация → применение данных.
